        ...
        function () {
            ...
            $.ajax({
                ...
                success: function (result){
                    ...
                    var alipay = data.aliPay.length;
                    var bankAccount = data.bankAccount.length;
                    console.log(alipay);
                    console.log(bankAccount);
                    if (alipay =="0"&&bankAccount=="0") {
        ...
        loadDownFn: function (me) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    var array = result.data.balanceRecords;
                    if(result.data.weekend){
                    	$("#weekErr").html("");
                    	$("#cash").show();
                    	  if (result.data.balance < result.data.payoutConfig) {
                              cashflag = false;
                              $("#cashErr").html("当前余额不足" + result.data.payoutConfig);
                          } else {
                              cashflag = true;
                              $("#cashErr").html("");
                          }
                    }else{
                    	$("#weekErr").html("非工作日方可申请提现！");
                    	$("#cash").hide();
                    }
                    $("#balance").html(result.data.balance)
                    $("#cash-rule").html("余额可以提现，提现金额必须是" + result.data.payoutConfig + "的倍数")
                    if (array.length <= 0) {
                        me.lock();// 锁定
                        me.noData();// 无数据
                    }
