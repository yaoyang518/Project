$(function() {
	...
	function loadData() {
		...
		$.ajax({
			...
			success : function(result) {
				...
				if (data.code == "0000") {
					var replenishRecords = data.replenishRecords;
					if (page == 1) {
						createPage(data.page);
					}
					$("#dataError").html("");
					$("#gh-scoreRecord-list").empty();
					for (var i = 0; i < replenishRecords.length; i++) {
						createTr(replenishRecords[i])
					}
	...
	function createTr(replenishRecord) {
		var tr = '<tr>';
		tr = tr + '<td>' + replenishRecord.id + '</td>';
		tr = tr + '<td><a href="/admin/user/view.html?id='
				+ replenishRecord.userId + '">' + replenishRecord.username
				+ '</a></td>';
		if (replenishRecord.minus) {
			tr = tr + '<td>减少</td>';
		} else {
			tr = tr + '<td>增加</td>';
		}
		tr = tr + '<td>' + replenishRecord.amount + '</td>';
		tr = tr + '<td>' + replenishRecord.createDate + '</td>';
		tr = tr + '</tr>';
